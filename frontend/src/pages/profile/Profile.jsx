...
export default function Profile() {
  ...
  async function sendRequest() {
    setReady(false);
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/friends/sendRequest`, {
        method: "post",
        credentials: "include",
  ...
  async function getMeetData() {
    if (dados.id != "") {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/meets/get/${dados.id}`,
          {
            credentials: "include",
          }
        );
  ...
  async function getUser() {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/user/getUser/${donoProfile}`,
        {
          credentials: "include",
        }
      );
      if (res.status != 200) {
        ...
        try {
          const res = await fetch(
            `${import.meta.env.VITE_BACKEND_URL}/api/friends/sentOrReceived`,
            {
              method: "post",
              credentials: "include",
              headers: { "Content-type": "application/json" },
  ...
  async function editUser() {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/user/edit/${user?.id}`,
        {
          method: "put",
          credentials: "include",
